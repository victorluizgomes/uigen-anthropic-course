...
function getToolLabel(tool: ToolInvocation, isDone: boolean): string {
  const args = (tool as { args?: Record<string, unknown> }).args;
  const command = args?.command as string | undefined;
  const path = (args?.path as string | undefined)?.replace(/^\//, '') ?? null;
  const newPath = (args?.new_path as string | undefined)?.replace(/^\//, '') ?? null;

  if (tool.toolName === "str_replace_editor") {
    switch (command) {
      case "view":
        return isDone ? `Read ${path ?? "file"}` : `Reading ${path ?? "file"}...`;
      case "create":
        return isDone ? `Created ${path ?? "file"}` : `Creating ${path ?? "file"}...`;
      case "str_replace":
      case "insert":
        return isDone ? `Updated ${path ?? "file"}` : `Updating ${path ?? "file"}...`;
      case "undo_edit":
        return isDone ? "Reverted changes" : "Reverting changes...";
      default:
        return isDone ? "File operation complete" : "Processing file...";
    }
  }
  ...
  if (tool.toolName === "file_manager") {
    switch (command) {
      case "rename":
        return isDone
          ? `Renamed to ${newPath ?? path ?? "file"}`
          : `Renaming ${path ?? "file"}...`;
      case "delete":
        return isDone ? `Deleted ${path ?? "file"}` : `Deleting ${path ?? "file"}...`;
      default:
        return isDone ? "File operation complete" : "Processing file...";
    }
  }
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{getToolLabel(tool, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">{getToolLabel(tool, false)}</span>
                                  </>
                                )}
                              </div>
                            );
